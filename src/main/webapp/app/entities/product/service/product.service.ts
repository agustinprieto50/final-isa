...
type RestOf<T extends IProduct | NewProduct> = Omit<T, 'dateAdded' | 'lastUpdated'> & {
...
export class ProductService {
  ...
  protected convertDateFromClient<T extends IProduct | NewProduct | PartialUpdateProduct>(product: T): RestOf<T> {
    return {
      ...
      lastUpdated: product.lastUpdated?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restProduct: RestProduct): IProduct {
    return {
      ...
      lastUpdated: restProduct.lastUpdated ? dayjs(restProduct.lastUpdated) : undefined,
    };
  }
